...
df3 = pd.read_csv("base_cno_tratada.csv", dicimal=',')

df.drop(['Cliente', 'Telefone(s)', 'Email','$ Venda' , '$ Locação', 'Nome do condomínio/edifício', 'Referências' , 'Usuário(s)'], axis=1, inplace=True)
...
df.drop('Região', axis=1, inplace=True)
...
if opcao == "Bairros Mais Procurados pelos Clientes":
    ...
    bairros_procurados = df['Bairros'].value_counts().head(10)
    ...
    construcao = df3[''].value_counts().head(10)
    fig4 = px.pie()
    ...
    barra = ["Sítio Cercado", "Pinheiro", "Cajuru", "Campo de santana", "Cidade industrial"]
    open = st.sidebar.selectbox("Escolha uma opção", barra)
    if open == "Por bairro":
        st.header("Por bairro")
        Imovel_mais_procurado_por_barro = df['Tipo'].value_counts().head(10)
        df_interesse = df[df['Tipo'].isin(Imovel_mais_procurado_por_barro)]
